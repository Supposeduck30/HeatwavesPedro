...
@Autonomous
@Configurable
public class Blue12BallCloseTurret extends OpMode {
    ...
    private Servo blocker; // Throughput blocker — replaces kicker

    // Blocker positions (mirrored from TeleOp)
    private static final double BLOCKER_BLOCK = 0.15;
    private static final double BLOCKER_OPEN  = 0.25;

    public double shootVelocity  = 1460;
    public double idleVelocity   = 1000;

    private static final double FIRE_TIME    = 0.41; // seconds per ball
    private static final double SPIN_UP_TIME = 1.0;  // seconds to reach velocity
    ...
    // Poses (preserved exactly from original)
    ...
    /**
     * Non-blocking throughput shoot. Opens blocker for FIRE_TIME per ball.
    ...
    public void loop() {
        ...
        panelsTelemetry.debug("Turret Position",   turret.getCurrentPosition());
